...
/******/ 	return hotCreateRequire("./docs/js/main.js")(__webpack_require__.s = "./docs/js/main.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony import */ var _pybossa__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./pybossa */ "./docs/js/pybossa.js");
/* harmony import */ var _utils_annotate__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../utils/annotate */ "./docs/utils/annotate.js");


let pathname = window.location.pathname;
let str = pathname.split('/');
let index = str.indexOf('project');
let projectname = str[index + 1];
let paragraphId = 0; //current sentence
// let totalParagraph = 4; //total sentence

let data = JSON.parse(localStorage.getItem('/annotateData')) || []; //Data json

let annotateData = JSON.parse(localStorage.getItem('/annotateHTML')) || []; //Data HTML
...
let paragraphsData = [{
  ...
  data: 'The standard chunk of Lorem Ipsum used since the 1500s is reproduced below for those interested.'
}, {
...
(function () {
  _utils_annotate__WEBPACK_IMPORTED_MODULE_1__["getLabels"].then(function (data) {
    if (data) {
      data.forEach(item => {
        let element = '';
        element += '<div class="controls">';
        element += '    <div class="tags">';
        element += '        <button class="tag value" data-color="' + item.color + '">' + item.value + '</button>';
        element += '    </div>';
        element += '</div>';
        $('#labelWrapper').append(element);
      });
    }
  }).then(() => addTagEventListener()).catch(err => console.log(err));
})(); // Run pybossa
...
function renderData() {
  ...
  if (oldAnnotates) {
    let annotate = oldAnnotates.find(e => e.id == paragraphId);
...
(function () {
  $('.deleteBtn').click(e => {
    ...
    deleteData(parent); // Delete span

    $(e.target).unwrap(); // // Delete button

    $(e.target).remove(); // // Save HTML
...
function addTagEventListener() {
  ...
  [].forEach.call(tags, tag => {
    let color = tag.getAttribute('data-color'); // Change tag background color

    tag.style.background = color;
    tag.addEventListener('click', function () {
      let selectRange = window.getSelection && window.getSelection();

      if (selectRange && selectRange.rangeCount > 0) {
        // Get selection range
        let range = window.getSelection().getRangeAt(0); // Create Tag

        let element = document.createElement('span');
        element.setAttribute('data-type', tag.innerText);
        element.classList.add('annotate');
        element.style.background = color; // Create delete button

        let button = $('<button class="deleteBtn">x</button>'); // Add tag

        range.surroundContents(element);
        $(element).append(button); // Add event delete
        ...
        button.on('click', () => {
          ...
          button.unwrap(); // Delete button

          button.remove(); // Save HTML

          saveAnnotation();
        }); // Save HTML

        saveAnnotation(); // Save data

        let type = tag.innerText;
        let text = $(element).text();
        let value = editValue(text);
        data.push({
          type,
          value,
          id: paragraphId
        });
        localStorage.setItem('/annotateData', JSON.stringify(data)); // Clear selection
...
function deleteData(parent) {
  let newData = JSON.parse(localStorage.getItem('/annotateData')); // Delete data

  let temp = {
    type: parent.attr('data-type'),
    value: editValue(parent.text()),
    id: paragraphId
  };
  let result = newData.filter(e => JSON.stringify(e) !== JSON.stringify(temp));
  data = result;
  localStorage.setItem('/annotateData', JSON.stringify(result));
}
...
function saveAnnotation() {
  ...
  if (hasAnnotate > 0) {
    let temp = {
      id: paragraphId,
      innerHTML: document.getElementById('content-wrapper').innerHTML
    };

    if (annotateData.length > 0) {
      let annotate = annotateData.find(e => e.id == paragraphId);

      if (annotate) {
        annotate.innerHTML = temp.innerHTML;
      } else {
        annotateData.push(temp);
      }
    } else {
      annotateData.push(temp);
    }
  ...
  localStorage.setItem('/annotateHTML', JSON.stringify(annotateData));
}
...
const pybossa = {
  // Public methods
  newTask: function (projectname) {
    return _getProject(projectname).then(_getNewTask);
  },
  saveTask: function (taskId, answer) {
  ...
  userProgress: function (projectname) {
    return _userProgress(projectname);
  },
  run: function (projectname, _window) {
...
var getLabels = new Promise(function (resolve, reject) {
  ...
  let index = str.indexOf('project');
  let projectname = str[index + 1];
  fetch(`/project/${projectname}/tasks/getlabels`).then(function (res) {
    return res.json();
  }).then(function (data) {
    if (data) {
      resolve(data);
...
function highlightAnnotation(e) {
  ...
  select.id = 'entity';
  let disableOpt = document.createElement('option');
  ...
  let clientY = e.clientY + scrollTop;
  annotation.style.top = `${clientY}px`;
  annotation.style.left = `${e.clientX}px`;
  return annotation;
} // Update Label

function updateLabel(annotation) {
  let textWidth = document.querySelector('#a_' + annotation.uuid + ' text').getBBox().width;
  let textHeight = document.querySelector('#a_' + annotation.uuid + ' text').getBBox().height;
  let newY = annotation.y - 7 - textHeight;
...
function getColorByEntityName(entity) {
  if (entity != undefined) {
    var temp = labels.find(e => e.key === entity);
